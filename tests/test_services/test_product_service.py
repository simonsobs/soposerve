...
async def test_get_existing_file(created_full_product, database, created_user):
    selected_product = await product.read_by_id(created_full_product.id,user = created_user)
...
async def test_get_missing_file(database, created_user):
    with pytest.raises(product.ProductNotFound):
        await product.read_by_name(name="Missing Product", version=None, user = created_user)

    with pytest.raises(product.ProductNotFound):
        await product.read_by_id(id=PydanticObjectId("7" * 24),user = created_user)

    with pytest.raises(product.ProductNotFound):
        await product.read_by_id("abcdefghijk",user = created_user)


@pytest.mark.asyncio(loop_scope="session")
async def test_add_to_collection(created_collection, created_full_product, database,created_user):
...
async def test_update_metadata(created_full_product, database, storage,created_user):
    ...
    with pytest.raises(versioning.VersioningError):
        await product.update_metadata(
            metadata, None, None, None, None,None, level=versioning.VersionRevision.MAJOR
        )
...
async def test_update_sources(created_full_product, database, storage, created_user):
    ...
    # mutated.
    created_full_product = await product.walk_to_current(product=created_full_product,user=created_user)
    ...
    new_product = await product.read_by_name(created_full_product.name, version=None,user = created_user)
...
@pytest.mark.asyncio(loop_scope="session")
async def test_update_sources_failure_modes(created_full_product, database, storage,created_user):
    created_full_product = await product.walk_to_current(product=created_full_product,user=created_user)
...
async def test_read_most_recent_products(database, created_user, storage):
    ...
    for i in range(20):
        await product.delete_tree(
            product=await product.read_by_name(name=f"product_{i}", version=None,user = created_user),
            storage=storage,
            data=True,
        )
...
async def test_walk_history(database, created_full_product,created_user):
    latest = await product.walk_to_current(created_full_product,user = created_user)
...
async def test_add_relationships(database, created_user, created_full_product, storage):
    ...
    # of the child.
    secondary_product = await product.read_by_id(secondary_product.id,user = created_user)
    original_product = await product.read_by_id(created_full_product.id,user = created_user)
    ...
    await product.delete_tree(
        product=await product.read_by_name(name=secondary_product.name, version=None,user = created_user),
        storage=storage,
        data=True,
    )
...
async def test_product_middle_deletion(database, created_user, storage):
    ...
    initial = await product.read_by_id(initial.id,user = created_user)
    final = await product.read_by_id(final.id,user = created_user)
