...
def test_api_products_for_use(test_api_client: TestClient, test_api_user: str):
    ...
    for name in product_names:
        response = test_api_client.delete(
            f"/product/delete/{name}", params={"data": True}
        )
        assert response.status_code == 200

    response = test_api_client.delete(
        f"/relationships/collection/delete/{collection_name}"
    )
    assert response.status_code == 200
...
def test_add_to_non_existent_collection(test_api_client):
    response = test_api_client.put(
        "/relationships/collection/add/Nonexistent/doesnt_matter"
    )
    assert response.status_code == 404

    # Also test removal
    response = test_api_client.delete(
        "/relationships/collection/remove/Nonexistent/doesnt_matter"
    )
    assert response.status_code == 404
...
def test_add_non_existent_product_to_existing_collection(
    ...
    response = test_api_client.put(
        f"/relationships/collection/add/{test_api_products_for_use[0]}/non_existent"
    )
    assert response.status_code == 404

    # Also test removal
    response = test_api_client.delete(
        f"/relationships/collection/remove/{test_api_products_for_use[0]}/non_existent"
    )
    assert response.status_code == 404
...
def test_add_child_product_to_non_existent_parent(test_api_client):
    response = test_api_client.put(
        "/relationships/product/add/child/Nonexistent/doesnt_matter"
    )
    assert response.status_code == 404
...
def test_remove_child_product_from_non_existent_parent(test_api_client):
    response = test_api_client.delete(
        "/relationships/product/remove/child/Nonexistent/doesnt_matter"
    )
    assert response.status_code == 404
...
def test_add_related_product_to_non_existent_product(test_api_client):
    response = test_api_client.put(
        "/relationships/product/add/related/Nonexistent/doesnt_matter"
    )
    assert response.status_code == 404
...
def test_remove_related_product_from_non_existent_product(test_api_client):
    response = test_api_client.delete(
        "/relationships/product/remove/related/Nonexistent/doesnt_matter"
    )
    assert response.status_code == 404
